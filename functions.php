 ...
 *
 * Requires WordPress 6.8+ and PHP 8.2+ (see style.css header).
 */
...
/** Capability for Theme Settings menu, ACF options pages, and Yak import/export AJAX. */
define( 'YAK_CAP_THEME_SETTINGS', 'yak_manage_theme_settings' );

/**
 * Master switch for Yak developer-mode *output* (Genesis hook markers, dev body class).
 * When false, the Theme Settings “Developer Mode” ACF toggle has no effect on rendered output.
 * Set true in wp-config.php on local/trusted environments only: define( 'YAK_DEBUG', true );
 */
if ( ! defined( 'YAK_DEBUG' ) ) {
	define( 'YAK_DEBUG', false );
}
...
if ( ! function_exists( 'get_field' ) ) {
	...
	// Special override: let a privileged user force a theme switch (ACF missing recovery).
	if ( is_admin() && isset( $_GET['yak_force_switch'] ) && current_user_can( 'switch_themes' ) ) {
...
/**
 * Unregister Genesis sidebars and stop primary sidebar output (single place; runs after Genesis registers widgets).
 */
add_action( 'after_setup_theme', 'yak_remove_genesis_sidebar_output', 15 );
function yak_remove_genesis_sidebar_output() {
	remove_action( 'genesis_sidebar', 'genesis_do_sidebar' );
}

add_action( 'widgets_init', 'yak_unregister_genesis_sidebars', 11 );
function yak_unregister_genesis_sidebars() {
...
function yak_theme_setup_features() {
	...
	add_editor_style( 'css/yak-editor-bridge.css' );      // Aligns editor canvas with theme.json + Yak tokens (see also inc/yak-colors.php)
...
require_once get_stylesheet_directory() . '/inc/yak-theme-lifecycle.php';
...
function clb_custom_admin_styles() {
	wp_enqueue_style(
		...
		[],
		filemtime( get_stylesheet_directory() . '/css/clb-custom-yak-admin-styles.css' )
	);
}
...
/**
 * Whether to load theme JS/CSS on this front request (skip feeds/embeds; filterable).
 */
function yak_should_load_frontend_theme_scripts() {
	if ( is_feed() || is_embed() ) {
		return false;
	}
	return (bool) apply_filters( 'yak_should_load_frontend_theme_scripts', true );
}

/**
 * Load frontend JS scripts and layout enhancements (conditional for performance).
 */
add_action( 'wp_enqueue_scripts', 'clb_enqueue_custom_scripts_styles', 100 );
function clb_enqueue_custom_scripts_styles() {
	if ( ! yak_should_load_frontend_theme_scripts() ) {
		return;
	}
	...
	wp_enqueue_script(
		...
		[],
		filemtime( get_stylesheet_directory() . '/js/clb-custom-yak-scripts.js' ),
		true
	);
...
/**
 * Dashicons on the frontend only when WooCommerce loads styles that reference them.
 */
add_action( 'wp_enqueue_scripts', 'yak_maybe_enqueue_wc_dashicons', 12 );
function yak_maybe_enqueue_wc_dashicons() {
...
/**
 * Font Awesome kit — frontend and admin (companion plugins expect it always registered).
 * Version is null so WordPress does not append ?ver=; the kit script and font files are cached
 * by the browser per normal HTTP caching (Font Awesome CDN Cache-Control).
 */
...
function yak_enqueue_yakstrap() {
	...
	wp_enqueue_script(
		...
		[],
		filemtime( get_stylesheet_directory() . '/js/yakstrap.js' ),
		true
	);
}
...
function yak_get_search_form_html() {
	...
	<form role="search" method="get" class="yak-modal-search-form search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
		...
		<input type="search" id="yak-search-field" class="search-field" placeholder="<?php echo esc_attr_x( 'Search…', 'placeholder', 'yak' ); ?>" value="<?php echo esc_attr( get_search_query() ); ?>" name="s" />
...
function yak_show_id_column_content( $column_name, $post_id ) {
	if ( $column_name === 'post_id' ) {
		echo '<pre class="yak-admin-id-pill">' . esc_html( $post_id ) . '</pre>';
	}
}
...
function yak_render_dashboard_widget() {
	...
	echo '<div class="yak-dash-welcome">';
	echo '<img class="yak-dash-welcome__avatar" src="' . $img_url . '" alt="Chris Liu-Beers" width="90" height="90" loading="lazy">';
	echo '<div class="yak-dash-welcome__body">';
	echo '<p class="yak-dash-welcome__lead">Congratulations on your new website.</p>';
	echo '<p>If you have any questions, contact me:</p>';
	echo '<ul class="yak-dash-welcome__contact">';
	echo '<li><a href="mailto:' . antispambot( $contact_email ) . '">' . antispambot( $contact_email ) . '</a></li>';
	echo '<li><a href="tel:+19195760180">919.576.0180</a></li>';
	echo '<li><a href="' . esc_url( $contact_link ) . '" target="_blank" rel="noopener noreferrer">Tomatillo Design</a></li>';
	echo '</ul></div></div>';
}
...
add_action( 'acf/init', function () {
	if ( function_exists( 'acf_add_options_page' ) ) {
		...
		acf_add_options_page( [
			...
			'capability'  => YAK_CAP_THEME_SETTINGS,
			'redirect'    => false,
		] );
 ...
 *
 * Theme Settings (ACF options, Performance, import/export) use capability {@see YAK_CAP_THEME_SETTINGS}.
 * That capability is granted only to Yak-authorized users via yak_grant_yak_theme_settings_cap().
 *
 * Authorized users:
 * - Manual ID list (yak_get_manual_allowed_user_ids)
 * - YAK_PRIMARY_USER_ID
 * - ACF option yak_allowed_users
 */
...
/**
 * Grant Theme Settings capability only to Yak-authorized users (never broad manage_options).
 */
add_filter( 'user_has_cap', 'yak_grant_yak_theme_settings_cap', 10, 4 );
function yak_grant_yak_theme_settings_cap( $all_caps, $caps, $args, $user ) {
	if ( empty( $args[0] ) || $args[0] !== YAK_CAP_THEME_SETTINGS ) {
		return $all_caps;
	}
	...
	if ( yak_user_is_yak_authorized( $user ) ) {
		$all_caps[ YAK_CAP_THEME_SETTINGS ] = true;
	}
...
function yak_restrict_theme_settings_page_access() {
	...
	if ( in_array( $_GET['page'], $restricted_pages, true ) && ! current_user_can( YAK_CAP_THEME_SETTINGS ) ) {
		wp_die(
...
function yak_hide_theme_settings_menu_for_unauthorized() {
	...
	remove_submenu_page( 'theme-settings', 'yak-options-performance' );
}
...
// Override option keys via filter 'yak/tracklight/option_names' if they differ from defaults.
