	...
	acf_add_local_field_group([
		...
		'fields' => [
			...
			[
				...
				'name' => 'yak_dev_mode',
				'type' => 'true_false',
				'instructions' => 'Requires define( \'YAK_DEBUG\', true ); in wp-config.php. When both are on, authorized users see Genesis hook markers and a dev body class. Toggle alone does nothing without YAK_DEBUG (prevents accidental production leakage).',
...
function yak_output_theme_favicons() {
	...
	if ( $favicon_url ) {
		...
		if ( is_array( $favicon ) && ! empty( $favicon['ID'] ) ) {
			$favicon_path = get_attached_file( (int) $favicon['ID'] );
			if ( $favicon_path && is_readable( $favicon_path ) ) {
				$favicon_ver = (string) filemtime( $favicon_path );
			}
		}
		if ( $favicon_ver === '' ) {
			$favicon_ver = wp_get_theme()->get( 'Version' ) ?: '1';
		}
		echo '<link rel="icon" href="' . esc_url( $favicon_url ) . '?v=' . rawurlencode( $favicon_ver ) . '" type="image/png">';
	}
...
function yak_add_dev_mode_body_class_admin_only_9991( $classes ) {
	if (
		yak_is_dev_mode_active() &&
		current_user_can( YAK_CAP_THEME_SETTINGS )
	) {
...
/**
 * Whether dev-mode UI should run: ACF toggle plus master YAK_DEBUG (see functions.php).
 */
function yak_is_dev_mode_active() {
	if ( ! defined( 'YAK_DEBUG' ) || ! YAK_DEBUG ) {
		return false;
	}
	return yak_is_dev_mode_enabled();
}

add_action( 'admin_notices', 'yak_dev_mode_acf_without_debug_constant_notice' );
function yak_dev_mode_acf_without_debug_constant_notice() {
	if ( ! current_user_can( YAK_CAP_THEME_SETTINGS ) ) {
		return;
	}
	if ( ! function_exists( 'get_field' ) || ! get_field( 'yak_dev_mode', 'option' ) ) {
		return;
	}
	if ( defined( 'YAK_DEBUG' ) && YAK_DEBUG ) {
		return;
	...
	echo esc_html__( 'Yak: Developer Mode is enabled in Theme Settings, but output is disabled until you set define( \'YAK_DEBUG\', true ); in wp-config.php (local/trusted sites only).', 'yak' );
	echo '</p></div>';
}
