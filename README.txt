...
= Version 1.0.5 =
* Requires WordPress 6.8+ and PHP 8.2+ (style.css header)
* Security: Theme Settings capability yak_manage_theme_settings; no broad manage_options grant for Yak-authorized users
* Security: yak_force_switch requires switch_themes; Performance & Tools restricted; import/export uses Theme Settings capability
* Output escaping for featured subtitle, search titles, search field value
* Dev-mode Genesis markers only for Theme Settings users; favicon cache param uses filemtime/version
* Performance: get_post_thumbnail_id for hero image meta
* Fix: Inline Genesis/Gutenberg appearance CSS on registered stylesheet handles
* Maintenance: Single sidebar teardown; speculation rules not forced on; import/export script logging removed
* Tooling groundwork: docs/tooling.md and .cursor/commands for future preflight/docs/update-ui
* Performance: Conditional front JS (skip feeds/embeds; mobile menu if Primary assigned); no jQuery for clb-custom-yak-scripts; filemtime yakstrap
* Dashicons on WC views only
* Font Awesome kit always on front + admin; null ver for CDN/browser cache
* theme.json + yak-editor-bridge.css + add_editor_style
* Import: yak_import_json_max_bytes, yak_import_denied_option_names, skip transients
...
Technical documentation for developers lives in the theme's `docs/` folder: `spec.php` (canonical behavior and requirements), `decisions.php` (why choices were made), `dev-notes.php` (how the code is organized). Markdown files in `docs/` are supplementary; `spec.php` is authoritative if they differ.
